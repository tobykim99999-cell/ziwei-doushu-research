    ...
    <main class="page-shell">
            ...
            <el-form class="chart-create-form" label-position="top" @submit.prevent>
                ...
                <div class="form-row">
                    <el-form-item label="出生日期">
                        <el-date-picker v-model="chartForm.birthDate" type="date" value-format="YYYY-MM-DD"
                            placeholder="选择日期" />
                    </el-form-item>
                    <el-form-item label="出生时辰">
                        <el-select v-model="chartForm.birthHour" placeholder="选择时辰">
                            <el-option v-for="hour in hourBranches" :key="hour.value"
                                :label="`${hour.label} ${hour.range}`" :value="hour.value" />
                        </el-select>
                    </el-form-item>
                </div>
                <div class="form-row">
        ...
        <section class="workbench content-shell">
            <aside class="sample-panel">
                ...
                <button v-for="chart in sampleCharts" :key="chart.id" class="sample-item"
                    :class="{ active: chart.id === activeChart.id }" type="button" @click="selectChart(chart.id)">
                ...
                <div v-else class="custom-chart-list">
                    <button v-for="chart in userCharts" :key="chart.id" class="sample-item"
                        :class="{ active: chart.id === activeChart.id }" type="button" @click="selectChart(chart.id)">
                        <strong>{{ chart.name }}</strong>
                        <span>{{ chart.birthSummary }}</span>
                    </button>
            ...
            <section class="chart-panel">
                ...
                <div class="palace-board">
                    <svg class="palace-relation-lines" viewBox="0 0 4 4" preserveAspectRatio="none" aria-hidden="true">
                        <line v-for="line in activeTriadLines" :key="line.key" x1="2" y1="2" :x2="line.point.x"
                            :y2="line.point.y" />
                    </svg>
                    <button v-for="palace in activeChart.palaces" :key="`${palace.branch}-${palace.name}`"
                        class="palace-cell" :class="{
                        ...
                        }" :style="{ gridArea: palace.area }" type="button" @click="activePalace = palace">
            ...
            <aside class="analysis-panel">
                ...
                <el-collapse class="score-collapse">
                    <el-collapse-item v-for="item in scoreAnalysisItems" :key="item.key"
                ...
                <el-button v-if="activeChart.source === 'user'" class="delete-chart-button" text type="danger"
                    :icon="Delete" @click="removeUserChart(activeChart.id.replace('user-', ''))">
                    删除当前命盘
                </el-button>
            </aside>
        </section>
        ...
        <section class="content-shell detailed-reading-panel">
            ...
            <div class="palace-reading-callout">
                ...
                <div>
                    <span>当前选中宫位补充</span>
                    <p>
                        {{ activePalace.name }}落{{ activePalace.branch }}位，主星为{{ formatStars(activePalace.majorStars)
                        }}，杂曜为{{ formatStars(activePalace.minorStars) }}。
            ...
            <div v-if="activeFlowTab !== 'patterns'" class="period-workbench">
                <aside class="period-selector">
                    <button v-for="(period, index) in currentPeriods" :key="period.key" type="button"
                        :class="{ active: index === getActivePeriodIndex() }" @click="setActivePeriod(index)">
                ...
                <section class="period-detail-panel">
                    ...
                    <div class="flow-focus-list">
                        ...
                        <span v-if="selectedPeriod.flowStars.length">流曜：{{ formatStars(selectedPeriod.flowStars)
                            }}</span>
                    ...
                    <div class="period-star-board">
                        <article v-for="palace in selectedFlowBoard" :key="`${palace.branch}-${palace.name}`"
            ...
            <div v-else class="chart-pattern-panel">
                ...
                <div class="pattern-guide">
                    <article>
                        <span>定格局</span>
                        <p>只按经典格局条件定名，先确认命宫、身宫和关键宫位是否满足成格要求，再看三方四正是否会齐所需星曜。</p>
                    </article>
                    <article>
                        <span>看破格</span>
                ...
                <div v-if="chartPatternAnalysis.structures?.length" class="structure-detail-block">
                    <div class="structure-detail-head">
                        <div>
                            <span>详细结构说明</span>
                            <strong>主星同宫、命身三方、四化与辅煞结构</strong>
                        </div>
                        <em>{{ chartPatternAnalysis.structures.length }} 项</em>
                    </div>
                    <el-collapse class="structure-collapse">
                        <el-collapse-item v-for="item in chartPatternAnalysis.structures" :key="item.name"
                            :title="`${item.name} · ${item.status}`">
                            <div class="structure-detail-card">
                                <p>{{ item.basis }}</p>
                                <article>
                                    <span>结构说明</span>
                                    <p>{{ item.reading }}</p>
            ...
            <div class="reading-footnote">
                ...
                <span>自定义命盘已接入 iztro 真实排盘；解读按本命定结构、三方四正看系统、四化追事件、大限/流年/流月/流时看触发来组织。系统样本仍保留 mock
                    展示，不同排盘软件可能因真太阳时、闰月、流派设置和起限规则不同而略有差异。</span>
            </div>
        </section>
    </main>
</template>
